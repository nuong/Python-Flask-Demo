...
def create_type(data):
    result = mongo.db.types.insert_one(data)
    type = mongo.db.books.find_one({'_id': _id(result.inserted_id)})
    return type


def list_types():
    results = mongo.db.types \
        .find()
    types = builtin_list(map(from_mongo, results))
    return types